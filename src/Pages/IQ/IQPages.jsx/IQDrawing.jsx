...
function IQDrawing({match}) {
	...
	useEffect(() => {
		...
		.collection('content').doc('drawing')
		.collection('details')
		.onSnapshot(snapshot => {
	...
	function handleSubmit(){
		...
		db.collection('IQ').doc(match.params.id)
		.collection('content').doc('drawing')
		.collection('details').add({title,desc,index,dno})
	}
	return (
		<div style={{color: '#43425D'}}>
			<IQHeader match={match}/>
			<br />
			<Typography align='center' variant='h1' gutterBottom><b>Drawing</b></Typography>
		...
		<Table  aria-label="simple table">
			...
			<TableCell align="left"><b className='text-lg font-bold italic'>Drawing Number</b></TableCell>
			<TableCell align="right"><b className='text-lg font-bold italic'>Actions</b></TableCell>
			...
			</TableHead>
					{
						contents.map(module => (
							...
							<>
							<DrawView module={module} match={match} key={module.id}/>	
	...
	<DialogContent>
		...
		<TextField style={{marginBottom: '3%'}} variant='outlined' label='Drawing Number' type='number' fullWidth onChange={(e) => setDno(e.target.value)}/>
