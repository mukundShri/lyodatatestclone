...
import SoftView from "../IQComponents/SoftView";

function IQSoftware({match}) {
	...
	function handleSubmit(){
		...
		db.collection('IQ').doc(match.params.id)
		.collection('content').doc('software')
		.collection('software').add({title,desc,index})
	}
	return (
		<div>
			...
			<TableCell align="right"><b className='text-lg font-bold italic'>Actions</b></TableCell>
			...
			</TableHead>
					{
						contents.map(module => (
							...
							<>
							<SoftView module={module} match={match} key={module.id}/>	
