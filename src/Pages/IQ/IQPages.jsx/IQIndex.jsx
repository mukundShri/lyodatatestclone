...
function IQIndex({match}) {
	...
	useEffect(() => {
		...
		.collection('content').doc('contents')
		.collection('details')
		.onSnapshot(snapshot => {
	...
	function handleSubmit(){
		...
		db.collection('IQ').doc(match.params.id)
		.collection('content').doc('contents')
		.collection('details').add({title,desc,index})
	}
	return (
			...
			<TableCell align="right"><b className='text-lg font-bold italic'>Actions</b></TableCell>
			...
			</TableHead>
					{
						contents.map(module => (
							...
							<>
							<IQConfigView module={module} match={match} key={module.id}/>	
